...
class Teams:
    HEADER = ['yearID', 'lgID', 'teamID', 'franchID', 'divID', 'teams.Rank',
              'G', 'Ghome', 'W', 'L', 'DivWin', 'WCWin',
              'LgWin', 'WSWin', 'R', 'AB', 'H', '2B',
              '3B', 'HR', 'BB', 'SO', 'SB', 'CS', 'HBP',
              'SF', 'RA', 'ER', 'ERA', 'CG', 'SHO', 'SV', 'IPouts', 'HA', 'HRA',
              'BBA', 'SOA', 'E', 'DP', 'FP', 'name', 'park', 'attendance', 'BPF', 'PPF', 'teamIDBR', 'teamIDlahman45', 'teamIDretro']
    

    COL_TYPES = ['int', 'str', 'str', 'str', 'str', 'int', 'int', 'int', 'int', 'int', 'str', 'str',
                 'str', 'str', 'int', 'int','int','int','int','int','int','int','int','int','int','int',
                 'int','int','int','int','int','int','int','int','int','int','int','int','int','int',
                 'str', 'str', 'int', 'int', 'int', 'str', 'str', 'str']

    def __init__(self):
        self.app = app

    def view_teams(self, row, sort_by=None, order=None, exclude_null=True): 
        try:
    ...
    def delete_teams(self, row):
        try:
            ...
            cursor = db.cursor()
            data = list2dict(row, self.HEADER)
            delete_query = 'DELETE FROM teams WHERE '
            for i in range(len(self.HEADER)):
                if data[self.HEADER[i]] == 'None':
